...
// Verbatim Brushland Labs vendor driver (docs.brushlandlabs.com). Only structural changes:
// package + imports + public visibility (callers live in another package). Do not local-patch;
// track upstream. (Known upstream quirk: getScanDistance() stores status without the >>5 shift
// that getDistance() applies, so getStatus() is on a different scale after a scan read.)
/**
