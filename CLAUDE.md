...
- **Comments: default to none.** Code should explain itself through naming and structure — do not narrate it. Add a comment *only* when a competent reader (knows Java + the FTC SDK) would still get it wrong without one: a non-obvious **WHY** that, left unwritten, would get "cleaned up" straight back into a bug — a subtle ordering requirement or invariant, a hardware/SDK/vendor quirk, or a deliberate deviation from the obvious approach. Keep it to one line where possible. When in doubt, leave it out; `git log`/`git blame` is where history lives.
  - **Never write:** comments restating what the code does; Javadoc that just re-says the method name (`/** Sets the power. */`); section-header banners (`// ─── label ──`); history or changelog notes ("was X, now Y", "moved from...", "previously this..."); the rationale for a past fix or audit finding; commented-out code; `@param`/`@return` that add nothing beyond the signature.
  - **Fine to write:** a short Javadoc on user-facing public API where the name alone doesn't carry intent; a one-line WHY on a genuine footgun.
  - **Vendored code** (`architecture/prism/*`, `architecture/hardware/LaserRangefinder.java`) is exempt — never add, remove, or reformat its comments; it tracks upstream verbatim.
